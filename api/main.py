...
app.config['MAIL_USERNAME'] = 'michaelsuponev@gmail.com'
app.config['MAIL_DEFAULT_SENDER'] = 'michaelsuponev@gmail.com'
...
def send_key():
    ...
    for game in data["purchasedGames"]:
        generated_keys = []
        for _ in range(game["qty"]):
            generated_key = generate_key()
            generated_keys.append(generated_key)
        result.append({"name": game["name"], "keys": generated_keys})

    msg = Message("Steam Shop", recipients=[data["emailInputValue"]])
    msg.body = "Ваши ключи, купленные на Steam Shop:\n"

    for game in result:
        msg.body += game["name"] + ": "
        for n in range(len(game["keys"]) - 1):
            msg.body += game["keys"][n] + ", "
        msg.body += game["keys"][len(game["keys"]) - 1] + "."
        msg.body += "\n"
